...
npm run seed        # runs pending migrations, then creates demo teams/users/cases
npm run start:dev
```
...
`synchronize: true` is gone — schema changes now go through real TypeORM
migrations, in dev, test, and prod alike:

- **One schema, one path.** `backend/src/database/data-source.ts` (used by
  the CLI below) and `DatabaseModule` (the app's actual runtime connection)
  both import the same `ENTITIES` list from `database/entities.ts`, so a
  migration generated by the CLI is guaranteed to match what the app
  actually expects — there's no separate "what synchronize would have done"
  to drift out of sync with it.
- **`migrationsRun: true`** in `DatabaseModule` means any pending migration
  runs automatically on boot — `npm run start:dev`, `npm run seed`, and the
  e2e suite's `createTestApp()` (against its isolated in-memory DB) all take
  this same path. The test suite passing is itself proof the migrations
  produce a working schema, not just that `synchronize` used to.
- **Workflow**: change an entity, then
  ```bash
  npm run migration:generate src/database/migrations/DescriptiveName
  ```
  review the generated SQL (TypeORM diffs entity metadata against the
  live dev DB — always sanity-check what it produced before committing it),
  then restart the app (or `npm run migration:run`) to apply it. `npm run
  migration:revert` undoes the most recent migration.
- **The initial migration was generated against an empty database**, not
  diffed from the existing dev DB — it recreates the full schema from
  nothing, which is what a fresh clone or a CI environment actually needs.
  If your local `data/incident-case-mgmt.sqlite` predates this change
  (i.e. it was created by the old `synchronize: true`), delete it and run
  `npm run seed` once — same one-time reset this README has already asked
  for after past relation-shape changes, just for the last time for this
  particular reason.
- **Known limitation**: SQLite's `ALTER TABLE` support is limited enough
  that TypeORM implements most structural changes (adding a foreign key,
  changing a column type) via create-temp-table → copy rows → drop → rename
  — visible as `temporary_<table>` statements in a generated migration.
  That's standard, correct SQLite practice, not a bug, but it does mean a
  migration touching a large table takes a full table rewrite, worth
  knowing before this scales past a scaffold's amount of data.
